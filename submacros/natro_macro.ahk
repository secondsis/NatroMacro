...
MainGui.Add("Button", "x10 y82 w150 h40 vBitterberryFeederButton Disabled", "Bitterberry`nAuto-Feeder").OnEvent("Click", nm_BitterberryFeederV2)
MainGui.Add("Button", "x10 y124 w150 h40 vAutoMutatorButton Disabled", "Auto-Jelly").OnEvent("Click", nm_AutoMutator)
;other tools
	...
	blc_start() {
		...
		While !stopping {
			...
			for i, j in selectedBees {
				...
				; Looking for warning icon to then use neonberry
...
nm_BitterberryFeederV2(*) {
	...
	if (MsgBox("Welcome to the BitterberryFeederV2 PROGRAM!``nMake sure BEE SLOT TO MUTATE is always visible``nDO NOT MOVE THE SCREEN OR RESIZE WINDOW FROM NOW ON.``nPress F11 to quit entirely, and press ESC to stop and reload the mutator UI.", "Bitterberry Auto-Feeder v2", 0x40001) = "Cancel")
		ExitApp

	bitterberrynos := InputBox("Enter the amount of bitterberry used each time", "How many bitterberry?", "w320 h180 T60").Value
	if IsInteger(bitterberrynos) {
		if (bitterberrynos > 30)
			if (MsgBox("You have entered " bitterberrynos " which is more than 30.``nAre you sure?", "Bitterberry Auto-Feeder v2", 0x40034) = "No")
				ExitApp
	} else {
		MsgBox "You must enter a number for Bitterberries!!``nStopping Feeder!", "Bitterberry Auto-Feeder v2", 0x40010
		ExitApp
	}

	if (MsgBox("After dismissing this message,``nleft click ONLY once on BEE SLOT", "Bitterberry Auto-Feeder v2", 0x40001) = "Cancel")
		ExitApp
	...
	FeedNeonberry() {
		...
		if (pos = 0) {
			MsgBox("Could not find neonberry")
			return
		}
		...
		Loop 10
		{
			...
			Gdip_DisposeImage(pBMScreen)
			if (A_Index = 10) {
				MsgBox ("Cannot find Feed button!")
				ExitFunc()
			}
		}
		Sleep 750
	...
	blc_start() {
		...
		if mgui is Gui
			mgui.hide()
		nextNeon := A_TickCount
		FeedNeonberry()

		While !stopping {
			ActivateRoblox()
			
			; Look for bitterberry
			if ((pos := nm_InventorySearch("bitterberry", "down", , , , (A_Index = 1) ? 40 : 4)) = 0)
			{
				MsgBox "You ran out of Bitterberries!", "Bitterberry Auto-Feeder v0.2", 0x40010
				break
			...
			GetRobloxClientPos(hwnd)
			; Move to and click at the bee slot position
			SendEvent "{Click " windowX+pos[1] " " windowY+pos[2] " 0}"
			Send "{Click Down}"
			Sleep 100
			SendEvent "{Click " beeX " " beeY " 0}"
			Sleep 100
			Send "{Click Up}"
			; For ten times, attempt to feed the bitterberries
			Loop 10
			{
				Sleep 100
				pBMScreen := Gdip_BitmapFromScreen(windowX+(54*windowWidth)//100-300 "|" windowY+offsetY+(46*windowHeight)//100-59 "|250|100")
			...
			sleep 800
			; No need to check for mythics or gifteds.


			; Do not need to check bees. Only check for mutations.

			; Waiting every 8 min to then use neonberry
			if (A_TickCount >= nextNeon) {
				global bitmaps
				FeedNeonberry()
				nextNeon := A_TickCount + 8*60*1000
			}
			...
			hBitmap := Gdip_CreateHBITMAPFromBitmap(pBitmap)
			pIRandomAccessStream := HBitmapToRandomAccessStream(hBitmap)
			text:= StrLower(RegExReplace(ocr(pIRandomAccessStream), "i)([\r\n\s]|mutation)*"))

			found := 0
			; CHECK FOR DESIRED MUTATION
			for i, j in selectedMutations
				for k, trigger in j.triggers
					if inStr(text, trigger) { 
						found := 1
						if inStr(text, "5movespeed") {
							MsgBox("Found 5 Movespeed")	
						}
						if inStr(text, "4%ability") {
							MsgBox("Found 4%% BAR")	
						}
						break
					}
			if (inStr(text, "ability") || inStr(text, "movespeed") || inStr(text, "attack") ){
				MsgBox("Found")
				found := 1
			}
