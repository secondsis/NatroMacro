...
nm_AutoMutatorOld(*) {
	...
	if (MsgBox("WELCOME TO THE AUTO MUTATOR PROGRAM!``nModded by snowedn``n``nMake sure BEE SLOT is always visible and NOT blocked by the Royal Jelly GUI``nDO NOT MOVE THE SCREEN OR RESIZE WINDOW FROM NOW ON.``nMAKE SURE AUTO-JELLY IS ON FOR MUTATIONS", "Auto Mutator Program", 0x40001) = "Cancel")
		ExitApp
	...
	blc_start() {
		...
		; Next neon use time (8 minutes). Start immediate on first loop
		nextNeon := A_TickCount
		While !stopping {
			ActivateRoblox()
			click windowX + Round(0.5 * windowWidth + 10) " " windowY + yOffset + Round(0.4 * windowHeight + 230)
			...
			; Every 8 minutes, check inventory and use a neonberry on the bee
			if (A_TickCount >= nextNeon) {
				; Ensure the inventory tab opens
				nm_OpenMenu("itemmenu")
				Sleep 300
				GetRobloxClientPos(hwnd)
				pCheck := Gdip_BitmapFromScreen(windowX "|" windowY+yOffset+72 "|350|80")
				if (Gdip_ImageSearch(pCheck, bitmaps["itemmenu"], , , , , , 5) != 1) {
					Gdip_DisposeImage(pCheck)
					MsgBox("Unable to open inventory (item menu). Aborting neon use"), "Auto Mutator Program", 0x40010
					break
				}
				Gdip_DisposeImage(pCheck)

				; Scroll until neonberry is found (scroll-to-end enabled, max attempts = 70)
				itemPos := nm_InventorySearch("neonberry", "up", , , , 70)
				if (itemPos = 0) {
					MsgBox ("You ran out of Neonberries!"), "Auto Mutator Program", 0x40010
					break
				}
				GetRobloxClientPos(hwnd)
								SendEvent "{Click " windowX+itemPos[1] " " windowY+itemPos[2] " 0}"
												Send "{Click Down}"
																Sleep 100
																				SendEvent "{Click " beeX " " beeY " 0}"
																								Sleep 100
